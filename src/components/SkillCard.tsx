...
export default function SkillCard({ skill, locale }: SkillCardProps) {
  ...
  const platformColors: Record<string, string> = {
    ...
    langchain: 'bg-green-500/20 text-green-300',
    autogpt: 'bg-purple-500/20 text-purple-300',
    'ai-tools': 'bg-cyan-500/20 text-cyan-300',
    dify: 'bg-pink-500/20 text-pink-300',
  };
    ...
    <Link href={`/${locale}/skills/${skill.id}`}>
      <div className="group relative rounded-xl border border-white/10 bg-gray-900/50 p-5 transition-all hover:border-white/20 hover:bg-gray-900/80 hover:shadow-lg hover:shadow-brand-500/5">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-3">
          <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${platformColors[skill.platform] || 'bg-gray-500/20 text-gray-300'}`}>
            {skill.platform}
          </span>
